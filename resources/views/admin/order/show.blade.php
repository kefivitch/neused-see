...
<div class="box">
		...
		<div class="box-header with-border">
			...
			<a title="Print Order" href="{{ route('order.print',$order->id) }}" class="btn btn-md btn-default pull-right"><i class="fa fa-print"></i></a>

			<a title="Edit Order" href="{{ route('admin.order.edit',$order->order_id) }}" class="margin-right-8 btn btn-md btn-success pull-right"><i class="fa fa-pencil"></i></a>		
		</div>
				...
				@if($order->refundlogs->count()>0)
					...
					@foreach($order->refundlogs->sortByDesc('id') as $rlogs)
						...
						<div class="callout callout-danger">
								...
								@php
									$orivar2 = App\AddSubVariant::withTrashed()->withTrashed()->findorfail($rlogs->getorder->variant_id);
				...
				<tbody>
					@foreach($order->invoices as $invoice)
						<tr>
							...
							<td>
								@php
                  				...
                  				<div class="row">
                  					...
                  					<div class="col-md-10">
                  						<a class="text-justify mleft22" target="_blank" title="Click to view" href="{{ url($url) }}"><b>{{substr($orivar->products->name, 0, 25)}}{{strlen($orivar->products->name)>25 ? '...' : ""}}</b>
								...
								<small class="mleft22"><b>Sold By:</b> {{$orivar->products->store->name}}</small>
									...
									<small class="mleft22"><b>Tax:</b> <i class="{{ $invoice->order->paid_in }}"></i>{{ round(($invoice->tax_amount/$invoice->qty),2) }}
							...
							<th>
								<a href="{{ route('print.invoice',['orderid' => $order->order_id, 'id' => $invoice->id]) }}" title="Print Invoice" href="#" class="btn btn-sm btn-default">

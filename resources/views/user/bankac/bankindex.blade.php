...
<!-- =========================small screen navigation end ============================ -->      
         ...
         <div class="col-md-12 col-xl-9">
            ...
            <div class="bg-white2 bg-white2-one">
              <a data-toggle="modal" data-target="#addnewbank" title="Add new bank account" class="text-white float-right btn btn-info">+ {{ __('staticwords.ADDNewbank') }}</a>
                <h5 class="user_m2">{{ __('staticwords.MyBankAccounts') }} ({{ $auth->banks->count() }})</h5>
                    ...
                    @foreach(Auth::user()->banks as $key=> $bank)
                      <tr>
                        ...
                        <td>
                          <div class="row">
                            <div class="col-md-2">
                                <button title="{{ __('Edit') }}" data-toggle="modal" data-target="#editbank{{ $bank->id }}" type="button" class="btn btn-sm btn-success ">
                                  <i class="fa fa-pencil"></i>
                                </button>
                            </div>
                            <div class="margin-left-5 col-md-2">
                                <button @if(env('DEMO_LOCK') == 0) data-toggle="modal" data-target="#deletebank{{ $bank->id }}" title="{{ __('Delete') }}" @else disabled="disabled" title="This action is disabled in demo !" @endif class="btn btn-sm btn-danger ">
...
@foreach(Auth::user()->banks as $key=> $bank)
    <div data-backdrop="static" data-keyboard="false" id="editbank{{ $bank->id }}" class="z-index99 delete-modal modal fade" role="dialog">
        ...
        <div class="modal-dialog modal-sm">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body text-center">
              ...
              <p>{{ __('Do you really want to delete this bank account') }} <b>{{ $bank->bankname }}</b>? {{ __('This process cannot be undone') }}.</p>
            </div>
            <div class="modal-footer">
...
<div data-backdrop="static" data-keyboard="false" class="z-index99 modal fade" id="addnewbank" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="form1" action="{{ route('pass.update',$user->id) }}" method="POST">
          ...
          <input required="" id="confirm_password" type="password" class="form-control" placeholder="{{ __('Re-enter password for confirmation') }}" name="password_confirmation" minlength="8"/>

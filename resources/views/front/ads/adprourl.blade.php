...
@php
   $pro = App\Product::find($isad->pro_id)
@endphp
@foreach($pro->subvariants as $key=> $orivar)
  @if($orivar->def ==1)
    ...
    @php 
        ...
        try{
         echo $link = url('details').'/'.$isad->pro_id.'?'.$name[0]['attr_name'].'='.$var_name[0].'&'.$name[1]['attr_name'].'='.$var_name[1];
        }catch(Exception $e)
        {
          echo $link = url(url('details').'/').'/'.$isad->pro_id.'?'.$name[0]['attr_name'].'='.$var_name[0];
        }
    @endphp

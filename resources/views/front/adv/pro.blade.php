@php
   $pro = App\Product::find($adv->pro_id1)
@endphp
@foreach($pro->subvariants as $key=> $orivar)
  @if($orivar->def ==1)
    ...
    @php 
        ...
        try{
         echo $link = url('details').'/'.$adv->pro_id1.'?'.$name[0]['attr_name'].'='.$var_name[0].'&'.$name[1]['attr_name'].'='.$var_name[1];
        }catch(Exception $e)
        {
          echo $link = url('details').'/'.$adv->pro_id1.'?'.$name[0]['attr_name'].'='.$var_name[0];
        }
    @endphp

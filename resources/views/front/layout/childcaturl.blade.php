@php
  $child = App\Grandcategory::where('id',$childid)->first();
  $price_array = array();
@endphp
...
@foreach($child->products as $old)
              ...
              @endforeach
                ...
                ?>
{{url('shop?category='.$child->category->id.'&sid='.$child->subcategory->id.'&chid='.$child->id.'&start='.round($startp*$conversion_rate,2).'&end='.round($endp*$conversion_rate,2))}}
@php
  $child = App\Grandcategory::where('id',$sliders->grand_id)->first();
  $price_array = array();
@endphp
...
@foreach($child->products as $old)
              ...
              @endforeach
                ...
                ?>
{{url('shop?category='.$child->category->id.'&sid='.$child->subcategory->id.'&chid='.$child->id.'&start='.$startp*$conversion_rate.'&end='.$endp*$conversion_rate)}}
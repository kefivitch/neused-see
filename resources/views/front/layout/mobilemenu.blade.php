@foreach($menus->where('status','=','1')->orderBy('position','ASC')->get() as $menu)
		...
		@if($menu->link_by == 'cat')
			...
			<li>
				...
				@endphp
				<a href="@if($menu->show_child_in_dropdown == 1 && $menu->linked_parent != NULL) @include('front.layout.caturl') @else @include('front.layout.caturl') @endif">@if($menu->icon != NULL || $menu->icon != '') <i class="fa {{ $menu->icon }}"></i>&nbsp;@endif{{ $menu->title }}

@php
  $s = App\Subcategory::where('id',$subcateid)->first();
  $price_array = array();
@endphp
...
{{url('shop?category='.$s->category->id.'&sid='.$s->id.'&start='.round($startp*$conversion_rate,2).'&end='.round($endp*$conversion_rate,2))}}